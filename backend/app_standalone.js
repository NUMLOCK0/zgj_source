...
const { createSqliteStore } = (function() {
  const clone = (v) => JSON.parse(JSON.stringify(v));
  const COLLECTIONS = ['users', 'customers', 'projects', 'tasks', 'consultStats', 'auditLogs', 'wukongSyncQueue'];
  const META_KEYS = ['nextCustomerId','nextUserId','nextTaskId','nextProjectId','stores','customerConfigs','nextConsultStatId','nextAuditLogId','taskIps','taskFps','wukongSync'];
  
  function normalize(defaultData, data) {
    const n = Object.assign(clone(defaultData), data || {});
    COLLECTIONS.forEach((k) => { if (!Array.isArray(n[k])) n[k] = []; });
    if (!Array.isArray(n.stores)) n.stores = clone(defaultData.stores || []);
    if (!n.taskIps || typeof n.taskIps !== 'object') n.taskIps = {};
    if (!n.taskFps || typeof n.taskFps !== 'object') n.taskFps = {};
    if (!n.wukongSync || typeof n.wukongSync !== 'object') n.wukongSync = clone(defaultData.wukongSync || {});
    if (!Array.isArray(n.wukongSyncQueue)) n.wukongSyncQueue = [];
    return n;
  }
  ...
  async function createSqliteStore(options) {
    ...
    let useSQLite = false;
    try { require('sql.js'); useSQLite = true; } catch(e) { console.log('sql.js不可用，使用JSON文件存储'); }
    ...
    if (useSQLite) {
      ...
      function run(sql, params) { const s = db.prepare(sql); try { s.run(params || []); } finally { s.free(); } }
      function allQ(sql, params) { const s = db.prepare(sql); const rows = []; try { s.bind(params || []); while (s.step()) rows.push(s.getAsObject()); } finally { s.free(); } return rows; }
      function scalar(sql, params) { const r = allQ(sql, params); return r.length ? Object.values(r[0])[0] : null; }
      run('CREATE TABLE IF NOT EXISTS app_meta (key TEXT PRIMARY KEY, value TEXT NOT NULL)');
      COLLECTIONS.forEach((n) => run('CREATE TABLE IF NOT EXISTS ' + n + ' (id INTEGER PRIMARY KEY, data TEXT NOT NULL)'));
      const isEmpty = COLLECTIONS.every((n) => Number(scalar('SELECT COUNT(*) FROM ' + n)) === 0);
      if (!exists || isEmpty) {
        let seed = clone(defaultData);
        if (fs.existsSync(jsonFile)) seed = normalize(defaultData, JSON.parse(fs.readFileSync(jsonFile, 'utf-8')));
        run('BEGIN TRANSACTION');
        try {
          COLLECTIONS.forEach((n) => { run('DELETE FROM ' + n); const s = db.prepare('INSERT INTO ' + n + ' (id, data) VALUES (?, ?)'); try { (seed[n] || []).forEach((item, i) => { s.run([Number.isInteger(item.id) ? item.id : i + 1, JSON.stringify(item)]); }); } finally { s.free(); } });
          run('DELETE FROM app_meta'); const ms = db.prepare('INSERT INTO app_meta (key, value) VALUES (?, ?)'); try { META_KEYS.forEach((k) => ms.run([k, JSON.stringify(seed[k])])); } finally { ms.free(); }
          run('COMMIT');
        } catch(err) { try { run('ROLLBACK'); } catch(e2){} throw err; }
        fs.writeFileSync(dbFile, Buffer.from(db.export()));
      }
      function readSnap() { const snap = clone(defaultData); COLLECTIONS.forEach((n) => snap[n] = allQ('SELECT data FROM ' + n + ' ORDER BY id').map((r) => JSON.parse(r.data))); META_KEYS.forEach((k) => { const v = scalar('SELECT value FROM app_meta WHERE key = ?', [k]); if (v !== null && v !== undefined) snap[k] = JSON.parse(v); }); return normalize(defaultData, snap); }
      function writeSnap(data) { run('BEGIN TRANSACTION'); try { COLLECTIONS.forEach((n) => { run('DELETE FROM ' + n); const s = db.prepare('INSERT INTO ' + n + ' (id, data) VALUES (?, ?)'); try { (data[n] || []).forEach((item, i) => { s.run([Number.isInteger(item.id) ? item.id : i + 1, JSON.stringify(item)]); }); } finally { s.free(); } }); run('DELETE FROM app_meta'); const ms = db.prepare('INSERT INTO app_meta (key, value) VALUES (?, ?)'); try { META_KEYS.forEach((k) => ms.run([k, JSON.stringify(data[k])])); } finally { ms.free(); } run('COMMIT'); } catch(err) { try { run('ROLLBACK'); } catch(e2){} throw err; } }
      return { dbFile, loadData: () => readSnap(), saveData: (d) => { writeSnap(normalize(defaultData, d)); fs.writeFileSync(dbFile, Buffer.from(db.export())); } };
    } else {
      let cached = null;
      if (fs.existsSync(jsonFile)) { try { cached = normalize(defaultData, JSON.parse(fs.readFileSync(jsonFile, 'utf-8'))); } catch(e) { cached = clone(defaultData); } } else { cached = clone(defaultData); }
      return { dbFile: jsonFile, loadData: () => cached, saveData: (d) => { cached = normalize(defaultData, d); fs.writeFileSync(jsonFile, JSON.stringify(cached, null, 2), 'utf-8'); } };
    }
  }
  return { createSqliteStore };
})();
const { syncCustomerToWukong, getWukongSyncSettings, updateWukongSyncSettings, fetchWukongToken, validateWukongToken } = (function() {
...
async function syncCustomerToWukong(customer, user, settings) {
  ...
  return { syncCustomerToWukong, getWukongSyncSettings, updateWukongSyncSettings, fetchWukongToken, validateWukongToken };
})();
