<template>
  <section class="customer-config-page finance-config-page">
    <div class="settings-section-title">财税配置</div>
    <el-card shadow="never" class="settings-card">
      <template #header>
        <div class="panel-head">
          <div>
            <div class="settings-row-label">财税字段与来源店铺</div>
            <div class="settings-row-desc">维护财税登记表中的店铺来源、业务类型和纳税人性质</div>
          </div>
        </div>
      </template>
...
const activeType = ref('stores')
const newValue = ref('')
const configs = ref({ stores: [], serviceTypes: [], taxpayerTypes: [] })
const configTypes = [
  { key: 'stores', label: '财税店铺' },
...
async function load() {
  ...
  configs.value = {
    stores: data.stores || [],
    serviceTypes: data.configs?.serviceTypes || [],
    taxpayerTypes: data.configs?.taxpayerTypes || []
  }
}
async function addValue() {
  ...
  if (!value) return
  try {
    const data = await financeApi.addConfig(activeType.value, value)
    configs.value[activeType.value] = data.values || []
    newValue.value = ''
    ElMessage.success('已新增配置')
  } catch (err) { ElMessage.error(err.message || '新增失败') }
}
async function removeValue(value) {
  try {
    await ElMessageBox.confirm(`确定删除配置“${value}”？`, '删除配置', { type: 'warning' })
    const data = await financeApi.removeConfig(activeType.value, value)
    configs.value[activeType.value] = data.values || []
...
onMounted(() => load().catch(err => ElMessage.error(err.message || '加载财税配置失败')))
</script>
